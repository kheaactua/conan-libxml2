...
import os
from conans.tools import download, unzip, replace_in_file, check_md5, vcvars_command, chdir
...
class LibxmlConan(ConanFile):
    ...
    requires = "zlib/1.2.8@lasote/stable", "libiconv/1.15@lasote/stable"
    ...
    def build_with_configure(self):
        env = AutoToolsBuildEnvironment(self, win_bash=(self.settings.os == "Windows"))
        env.fpic = True
        ...
        with chdir(self.ZIP_FOLDER_NAME):
            env.configure(args=['--with-python=no', '--without-lzma'])
            env.make()
